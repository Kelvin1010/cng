...
async function getMultiple(filterSpec = null, page = null, pageSize = null) {
  ...
  const rows = await Distance.findAll(params);
...
async function getById(id) {
  var result = await Distance.findByPk(id);
  return result;
}

async function create(prop) {
  var result = await Distance.create(prop);
  return {
...
async function update(id, tag) {
  const result = await Distance.update(tag, { where: { id } });
  return {
...
async function remove(id) {
  const result = await Distance.destroy({ where: { id } });
  return {
