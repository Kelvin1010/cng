...
async function getMultiple(filterSpec = null, page = null, pageSize = null) {
  ...
  const rows = await Customer.findAll(params);
...
async function getById(id) {
  var result = await Customer.findByPk(id);
  return result;
}

async function create(prop) {
  var result = await Customer.create(prop);
  return {
...
async function update(id, tag) {
  const result = await Customer.update(tag, { where: { id } });
  return {
