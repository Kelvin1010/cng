...
async function get(req, res, next) {
  try {
    ...
    const result = await customerService.getMultiple(filterSpec, page, pageSize);
    res.json(result);
  } catch (err) {
...
async function create(req, res, next) {
  try {
    const data = req.body;
    res.json(await customerService.create(data));
  } catch (err) {
...
async function update(req, res, next) {
  try {
    const id = req.params.id;
    const tag = req.body;
    res.json(await customerService.update(id, tag));
  } catch (err) {
...
async function remove(req, res, next) {
  try {
    const id = req.params.id;
    res.json(await customerService.remove(id));
  } catch (err) {
