...
async function getMultiple(filterSpec = null, page = null, pageSize = null) {
  ...
  const rows = await Point.findAll(params);
...
async function getById(id) {
  var result = await Point.findByPk(id);
  return result;
}

async function create(prop) {
  var result = await Point.create(prop);
  return {
...
async function update(id, tag) {
  const result = await Point.update(tag, { where: { id } });
  return {
...
async function remove(id) {
  const result = await Point.destroy({ where: { id } });
  return {
