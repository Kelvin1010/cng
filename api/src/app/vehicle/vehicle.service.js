...
async function getMultiple(filterSpec = null, page = null, pageSize = null) {
  ...
  const rows = await Vehicle.findAll(params);
...
async function getById(id) {
  var result = await Vehicle.findByPk(id);
  return result;
}

async function create(prop) {
  var result = await Vehicle.create(prop);
  return {
...
async function update(id, tag) {
  const result = await Vehicle.update(tag, { where: { id } });
  return {
